...
public class kwmsActivity14bAdapter extends BaseAdapter {
    private List<kwmsActivity14bModel> mData;
    private Context mContext;

    public kwmsActivity14bAdapter (Context context,List data){
        this.mData = data;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        View view =null;
        kwmsActivity14bAdapter.Holder holder = null;
        if(convertView != null){
            view = convertView;
            holder = (kwmsActivity14bAdapter.Holder)view.getTag();
        }else{
            view = View.inflate(mContext, R.layout.activity_14bitem, null);
            holder = new kwmsActivity14bAdapter.Holder();
            holder.task = view.findViewById(R.id.task);
            holder.state = view.findViewById(R.id.state);
            view.setTag(holder);
        }
        kwmsActivity14bModel  friend = mData.get(position);
