...
public class Activity10 extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public void onClick(View v) {
        if(v.equals(backBut)){
            ...
            Intent intent=new Intent(Activity10.this,Activity03a.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == 1){
            ...
            if(IsQuery){
                ...
                String topOrg = zzasacUserMUO.getOrgTop();//获取顶层机构
                GetOutlistData(topOrg,clientID,wahoID,"6");
            }else {
    ...
    private void SetOutlistData(wmsagOutlistBean weatherEntity){
        ...
        if(sum > 0){
            List<gwmsActivity10Model> lists = new ArrayList<>();
            ZZaswmsacBean = weatherEntity;
            for (wmsagOutlistBean.outData item : ZZaswmsacBean.getOutData()) {
                gwmsActivity10Model init = new gwmsActivity10Model();
                init.setId(item.getId());
                init.setName(item.getOutsum());
                lists.add(init);
            }
            gwmsActivity10Adapter mAdapter = new gwmsActivity10Adapter(Activity10.this,lists);
            lv_list.setAdapter(mAdapter);
        }else {
            lv_list.setAdapter(null);
            Toast.makeText(getApplicationContext(), "没有出库清单信息", Toast.LENGTH_SHORT).show();
        }
    }
    @Override
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        //跳转对应页面
        Intent intent=new Intent(Activity10.this,Activity10a.class);
