...
public class Activity03 extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public void onClick(View v) {
        if(v.equals(backBut)){
            ...
            Intent intent=new Intent(Activity03.this,Activity03a.class);
    ...
    private void SetEnterlistData(wmsacEnterlistBean weatherEntity){
        ...
        if(sum > 0){
            List<bwmsActivity03Model> lists = new ArrayList<>();
            ZZaswmsacBean = weatherEntity;
            for (wmsacEnterlistBean.outData item : ZZaswmsacBean.getOutData()) {
                bwmsActivity03Model init = new bwmsActivity03Model();
                init.setId(item.getId());
                init.setName(item.getEntersum());
                lists.add(init);
            }
            bwmsActivity03Adapter mAdapter = new bwmsActivity03Adapter(Activity03.this,lists);
            lv_list.setAdapter(mAdapter);
        }else {
            lv_list.setAdapter(null);
            Toast.makeText(getApplicationContext(), "没有入库清单信息", Toast.LENGTH_SHORT).show();
        }
    }
    //listview点击事件
    @Override
