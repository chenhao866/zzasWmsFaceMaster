...
public class Activity13 extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    private ImageButton backBut;
    private ListView lv_list;
    private SwipeRefreshLayout swipeRefreshLayout;//下拉刷新
    private wmsalShenBeiBean ZZaswmsacBean;
    @Override
    ...
    //执行获得库位调整商品信息接口
    protected void GetSheBeiData(String toporgid){
        ...
        rxjavaService .postMapSheBei(myMap)
                .observeOn(AndroidSchedulers.mainThread())//主线程显示数据
                .subscribeOn(Schedulers.io())//IO线程加载数据
                .subscribe(new DisposableObserver<wmsalShenBeiBean>() {
                    @Override
                    public void onNext(wmsalShenBeiBean weatherEntity){//获取数据后执行;WeatherEntity为自己定义的数据类
                        if (weatherEntity.getException() == null){
                            SetSheBeiData(weatherEntity);//根据返回数据显示下拉信息
                        }else {
    ...
    //根据接口数据显示listView
    private void SetSheBeiData(wmsalShenBeiBean weatherEntity){
        int sum = weatherEntity.getOutData().length;
        if(sum > 0){
            List<jwmsActivity13Model> lists = new ArrayList<>();
            ZZaswmsacBean = weatherEntity;
            for (wmsalShenBeiBean.outData item : ZZaswmsacBean.getOutData()) {
                jwmsActivity13Model init = new jwmsActivity13Model();
                init.setCheckType(item.getChecktype());
                init.setState(item.getState());
                lists.add(init);
            }
            jwmsActivity13Adapter mAdapter = new jwmsActivity13Adapter(Activity13.this,lists);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        Intent intent=new Intent(Activity13.this,Activity13a.class);
