...
public class Activity10a extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    private void SetOutcomlistData(wmsahOutcomlistBean weatherEntity){
        ...
        if(sum > 0){
            ...
            List<gwmsActivity10aModel> lists = new ArrayList<>();
            for (wmsahOutcomlistBean.outData item : weatherEntity.getOutData()) {
                gwmsActivity10aModel init = new gwmsActivity10aModel();
            ...
            gwmsActivity10aAdapter mAdapter = new gwmsActivity10aAdapter(Activity10a.this,lists);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        Intent intent=new Intent(Activity10a.this,Activity10b.class);
    ...
    private void verifyRecheckFun(){
        ...
        for (wmsahOutcomlistBean.outData item : ZZasOutcomlistBean.getOutData()) {
            if(item.getState() != null && item.getState().equals("1")){
                DialogFun("有已审核完成的数据，确定放弃出库审核订单吗?", "backBut");
                return ;
            }
        }
        finish();
    ...
    private Boolean NoVerifyRecheckFun(){
        ...
        for (wmsahOutcomlistBean.outData item : ZZasOutcomlistBean.getOutData()) {
            if(item.getState() == null || item.getState().equals("")){
                ...
                zzasdialog.showSimpleTipDialog(Activity10a.this,str,tag,YesClick);
                return false;
            }
        }
        return true;
    ...
    private String packageJsonDataFun(){
        ...
        ZZasmc_bean.setState("7");
        map.put("outlist",ZZasmc_bean);
        map.put("outcomlist",ZZasOutcomlistBean.getOutData());
        Gson gson = new Gson();
        String str = gson.toJson(map);
        return  str;
    }
    //执行入库请求接口
    protected void SetOutlistData(String JsonData){
        zzasadLoad.ShowZZASLoad(this,true);//加载框
        String url = new Appcofig().GetCofigData(this,"configIP");
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(url)//基础URL
        ...
        rxjavaService .postMapSetOutlist(body)
                ...
                .subscribe(new DisposableObserver<wmsafStolocBean>() {
                    ...
                    public void onNext(wmsafStolocBean weatherEntity){//获取数据后执行;WeatherEntity为自己定义的数据类
                        if (weatherEntity.getException() == null){
                            Toast.makeText(getApplicationContext(), "出库完成", Toast.LENGTH_SHORT).show();
                            finish();
                        }else {
                            String str = weatherEntity.getException().getMessage();
                            Toast.makeText(getApplicationContext(), str, Toast.LENGTH_SHORT).show();
    ...
    private void DialogFun(String str, String tag){
        zzasToolBdialog zzasdialog = new zzasToolBdialog();
        zzasdialog.showSimpleConfirmDialog(Activity10a.this,str,tag,YesClick,null);
    }
    //弹窗确定按钮点击事件
