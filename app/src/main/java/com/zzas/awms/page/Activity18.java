...
import com.baidu.aip.fl.LoginResultActivity;
...
public class Activity18 extends AppCompatActivity {
    ...
    private Boolean ShoeOff = true;//记录跳转次数
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initScreen();
        initView();
        mHandler = new Activity18.InnerHandler(this);
    ...
    private void initView() {
        ...
        cameraImageSource.getCameraControl().setPermissionCallback(new PermissionCallback() {
            ...
            public boolean onRequestPermission() {
                ActivityCompat.requestPermissions(Activity18.this,
...
//                Color.parseColor("#3cf16d7a"));
    ...
    private void initBrightness() {
        int brightness = BrightnessTools.getScreenBrightness(Activity18.this);
        if (brightness < 200) {
    ...
    private void upload(FaceFilter.TrackedModel model) {
        ...
        if (model.getEvent() != FaceFilter.Event.OnLeave) {
            ...
            try {
                ...
                ImageSaveUtil.saveCameraBitmap(Activity18.this, face, "head_tmp.jpg");
                ...
                APIService.getInstance().identify(new OnResultListener<RegResult>() {
                    ...
                    public void onResult(RegResult result) {
                        ...
                        if (result == null) {
                            mUploading = false;
                            if (mDetectCount >= 3) {
                                Toast.makeText(Activity18.this, "人脸校验不通过,请确认是否已注册", Toast.LENGTH_SHORT).show();
                                finish();
                            }
                            return;
                        }
                        ...
                        if (maxScore > 80) {
                            ...
                            mDetectTime = false;
                            Toast.makeText(Activity18.this, "人脸已注册，禁止重复注册", Toast.LENGTH_SHORT).show();
                            finish();
                            return;
                        } else {
                            ...
                            if (mDetectCount >= 3) {
                                mDetectTime = false;
                                Toast.makeText(Activity18.this, "采集成功", Toast.LENGTH_SHORT).show();
                                Intent intent = new Intent(Activity18.this, Activity18a.class);
                    ...
                    public void onError(FaceError error) {
                        ...
                        if (mDetectCount >= 3) {
                            ...
                            if (error.getErrorCode() == 10000) {
                                Toast.makeText(Activity18.this, "人脸校验不通过,请检查网络后重试", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(Activity18.this, "人脸校验不通过，请重试", Toast.LENGTH_SHORT).show();
                            }
                            finish();
                            return;
                        }
    ...
    private static class InnerHandler extends Handler {
        private WeakReference<Activity18> mWeakReference;

        public InnerHandler(Activity18 activity) {
            super();
            this.mWeakReference = new WeakReference<Activity18>(activity);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            Activity18 activity = mWeakReference.get();
            if (activity == null) {
                return;
            }
            if (msg == null) {
